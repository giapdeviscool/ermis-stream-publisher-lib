...
If you want to use Android Studio or Xcode to edit the native code, you can open the `example/android` or `example/ios` directories respectively in those editors. To edit the Objective-C or Swift files, open `example/ios/ErmisStreamPublisherLibExample.xcworkspace` in Xcode and find the source files at `Pods > Development Pods > ermis-stream-publisher-lib`.

To edit the Java or Kotlin files, open `example/android` in Android studio and find the source files at `ermis-stream-publisher-lib` under `Android`.
...
```sh
Running "ErmisStreamPublisherLibExample" with {"fabric":true,"initialProps":{"concurrentRoot":true},"rootTag":1}
```
